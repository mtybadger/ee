...
void main() {
	...
	vec3 lerpcolor = vec3((1.- col.r) * vec3(0.99, 0.82, 0.75));
	if(col.r > 0.11){
		lerpcolor = lerp(vec3(1., 0.41, 0.41), vec3(0.713, 0.09, 0.09), pow(col.r, 0.5));
	}
	if(col.r > 0.3){
		lerpcolor = lerp(vec3(0.01, 0.14, 0.28), vec3(0.85), 0.);
	}

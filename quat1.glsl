...
float hitBrot(vec3 point){
		...
		vec4 q1 = normalize(vec4(1., 0., 0., 0.));
		vec4 q2 = normalize(vec4(cos(-3.141/4.), 0., sin(3.141/4.), 0.));
    	c = qmul(qmul(q1, c), q2);
			// iterate
			float di =  1.0;
			vec4 z  = vec4(0.0);
...
void main() {
    ...
    Ray ray = Ray(vec3(-0.13, 0.0, -2.), 0.2*normalize(vec3(uv, 1.)));
    ...
    for (int i=0; i<256; i++) {
    	...
    	if (dist < 0.0001) {
			...
			float iterationScalar = min((1.0 / (float(i)/37.)), 0.7);
	...
	vec3 lerpcolor = vec3(col.r * vec3(0.0, 0., 0.447));
		if(col.r > 0.2){
			lerpcolor = lerp(vec3(0.2, 0.08, 0.46), vec3(0.639, 0.184, 0.5), pow(col.r, 0.5) + 0.2);
		}
		if(col.r > 0.35){
			lerpcolor = lerp(vec3(1., 0.388, 0.388), vec3(0.85), 0.);
		}

...
void main() {
    ...
    Ray ray = Ray(vec3(-0.5, 0.0, -2.0 + (sin(u_time) * 0.0)), normalize(vec3(uv, 1.)));
	...
	vec3 lerpcolor = vec3(col.r * vec3(0.21, 0.07, 0.376));
	if(col.r > 0.0){
		lerpcolor = lerp(vec3(0.4471, 0.0, 0.2157), vec3(0.996, 0.203, 0.431), pow((col.r*4.),2.)/1. + 0.2);
	}
	if(col.r > 0.3){
		lerpcolor = lerp(vec3(1., 0.741, 0.411), vec3(0.85), 0.);
	}
